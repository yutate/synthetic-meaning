...
2. **Persona Interpretation** — 5種のSynthetic Personaが3タブで多角的に反応
   - **第一印象** — 初見での率直な解釈・魅力・不安・7軸スコア
   - **Meaning Timeline** — 1〜3秒 / 5〜10秒 / 15〜30秒 / 30〜60秒の時系列変化、Exit Point可視化
   - **Exit Point** — 離脱タイミング・Exit Type・引き金・改善案
3. **仮想ユーザー座談会** — Persona同士が議論し、解釈の対立・共通認識を可視化
4. **Memory Retention** — 座談会後→翌日、各Personaに何が記憶として残ったかを可視化（Memory Stability Score）
5. **Meaning Diagnosis レポート** — 訴求の強み・意味のズレ・改善提案・Attention Decay Graph・次に検証すべき論点

## Exit Types

| Type | 意味 |
|---|---|
| Cognitive Overload | 情報量・条件が多すぎて離脱 |
| Trust Breakdown | 怪しさ・条件の不透明さで離脱 |
| Low Relevance | 自分向けではないと判断して離脱 |
| Emotional Rejection | 世界観・デザインが合わず離脱 |
| Attention Exhaustion | 読み続ける気力がなくなって離脱 |
| No Exit | 最後まで閲覧 |
...
- **v1.1** — Meaning Timeline / Exit Point Detection / Memory Retention / Attention Decay Graph 追加（6ステップ構成へ）
- **v1.0** — 初期リリース
